...
(function () {
  describe('createMac', function () {
    ...
    it('should validate only arguments', function () {
      var valid = MacAttack.validateMac(arrayObjectLengthMac, "my secret", [{a: 1}, {a: 2}, {a: 3}]);
      expect(valid).toBe(true);
    });

    it('should validate to true if proper mac is in signature', function () {
      var valid = MacAttack.validateMac(numGetterConsumerMac, "my secret", {a: 1, b: numGetterMac});
      expect(valid).toBe(true);
    });

    it('should not validate to true if wrong signature mac is in signature', function () {
      var valid = MacAttack.validateMac(stringGetterNotConsumerMac, "my secret", {a: 1, b: numGetterMac});
      expect(valid).toBe(false);
    });
      
    it('should check valid signatures of macaroons in request json', function () {
      var valid = MacAttack.validateMac(numGetterReturningConsumerMac, "my secret", {a: 1, b: numGetterReturningMac});
      expect(valid).toBe(true);
    });

    it('should check invalid signatures of macaroons in request json', function () {
      var valid = MacAttack.validateMac(numGetterReturningConsumerMac, "my secret", {a: 1, b: stringGetterReturningMac});
      expect(valid).toBe(false);
    });
